...
BO_skel_check <- BO_merged_df5 %>% 
  filter(!is.na(AG_biomass_mg))
#210 vs 209, one skel missing, This skeleton came from a plant that never flowered or fruited, so would not be included in the data anyways.

#how to check what maternal ID didn't match up
BO_skel_check_2 <- anti_join(skel_weights_all2, BO_skel_check, by = c("Transect", "Recipient", "AG_biomass_mg")) %>% 
  select(AG_biomass_mg, Transect, Recipient)
```
...
BO_corolla_check <- BO_merged_df6 %>% 
  filter(!is.na(corolla_diam_mm)) #507 obs
...
BO_leaves_check <- anti_join(leaves_BO_ind, BO_NFleaves, by = c("Block", "Transect", "Recipient", "leaf_dryweight_mg")) %>% 
...
#262 observations but all are supposed to be NAs bc no closed fruits
...
d13C_check_BO <- BO_merged_df_fl3b %>% 
  filter(FitP == TRUE) %>% 
  filter(!is.na(d13C_SEG)) #196 observations in both df's
...
write_csv(BO_mastersheet_v3, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_full_2022.csv"))
...
write_csv(BO_mastersheet_F_mains, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_Fitness-mains_2022.csv"))

write_csv(BO_mastersheet_F_all, here::here("data_sheets", "compiled_sheets", "BO_mastersheet_Fitness-all_2022.csv"))
