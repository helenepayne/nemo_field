...
corolla_model <- lmer(corolla_diam_mm_SEG ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
## Create the mixed model for corolla area that includes transect
corolla_model_2 <- lmer(corolla_diam_mm_SEG ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
group_means <- AC_22_23_fit %>%
  group_by(Transect) %>%
  summarise(mean_corolla_diam = mean(corolla_diam_mm_SEG, na.rm = TRUE))

#Mean center the variable by Transect
AC_22_23_fit <- AC_22_23_fit %>%
  left_join(group_means, by = "Transect") %>%
  mutate(corolla_diam_mm_SEG_centered = corolla_diam_mm_SEG - mean_corolla_diam) %>%
  select(-mean_corolla_diam)  # Optional: Remove the mean_corolla_diam column
#####

#Fit the model using the mean-centered variable
corolla_model_3 <- lmer((corolla_diam_mm_SEG_centered) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
corolla_model_comparison <- data.frame(
  Model = c("corolla_model", "corolla_model_2", "corolla_model_3"),
  AIC = c(AIC(corolla_model), AIC(corolla_model_2), AIC(corolla_model_3)),
  BIC = c(BIC(corolla_model), BIC(corolla_model_2), BIC(corolla_model_3)),
  LogLikelihood = c(logLik(corolla_model), logLik(corolla_model_2), logLik(corolla_model_3))
)

# Print model comparison
print(corolla_model_comparison)

#model 3 appears the best
```
...
skel_model <- lmer(log(skel_dryweight_mg_SEG) ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
# Create the mixed model for skeleton weight, with skeleton weight log transformed
skel_model_2 <- lmer(log(skel_dryweight_mg_SEG) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
#Fit the model using the mean-centered variable
skel_model_3 <- lmer((skel_dryweight_mg_SEG_centered) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)

# # Check if the model is singular
isSingular(skel_model_3) #TRUE, likely due to over-fitting, will not include
```
...
#model 1 and 2 appear pretty similar, use one of those
...
fl_duration_model <- lmer(fl_duration ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
est_fecundity_model <- lmer(sqrt(est_fecundity) ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
# Create the mixed model for estimated fecundity, sqrt transforming estimated fecundity
est_fecundity_model_2 <- lmer(sqrt(est_fecundity) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
#Fit the model using the mean-centered variable
est_fecundity_model_3 <- lmer((est_fecundity_centered) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
msm_model <- lmer(log(msm_all) ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
# Create the mixed model for mean seed mass, log transformed mean seed mass
msm_model_2 <- lmer(log(msm_all) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
#Fit the model using the mean-centered variable
msm_model_3 <- lmer((msm_centered) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
SLA_model <- lmer(SLA_SEG ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
# Create the mixed model for SLA
SLA_model_2 <- lmer(SLA_SEG ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)

# Check if the model is singular
isSingular(SLA_model_2) #TRUE, likely due to over-fitting, will not include
...
```{r}
# Create the mixed model for mean seed mass, log transformed mean seed mass
LMA_model <- lmer(LMA_SEG ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
d13C_model <- lmer(d13C_SEG ~ (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
# Create the mixed model for d13C, log transformed mean seed mass
d13C_model_2 <- lmer(d13C_SEG ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
#Fit the model using the mean-centered variable
d13C_model_3 <- lmer((d13C_SEG_centered) ~ Transect + (1 | Recipient) + (1 | Donor), data = AC_22_23_fit)
...
corolla_variances <- calculate_variances(corolla_model_3, "corolla_diam_mm")
...
```{r}
#Save the csv file
write_csv(x = variance_components_AC, here::here("data_sheets", "compiled_sheets", "AC_Va_h2_R.csv"))
