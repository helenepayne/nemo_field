...
# Function to Get variance components and calculate required values
calculate_variances <- function(model, trait_name) {
  ...
  if ("Recipient" %in% var_components$grp) {
    Va_mat <- (var_components$vcov[var_components$grp == "Recipient"]) * 4
    Va_sd_mat <- sqrt(Va_mat)
  }
  
  # Calculate other variance components
  if ("Donor" %in% var_components$grp) {
    Va_pat <- (var_components$vcov[var_components$grp == "Donor"]) * 16
