...
# Read in the dataset and mutate columns correctly
G0_msm <- read_csv(here::here("data_sheets", "seed_mass", "g0_seeds.csv")) %>%
  # Rename columns
  rename(Recipient = Pop_ID, Donor = `Sire ID`, g0_msm = `Mean Individual Seed Mass (g)`) %>%
  
  # Separate and pad the 'Recipient' column
  separate(Recipient, into = c("Rec_Pop", "Rec_ID"), sep = "_") %>%
  mutate(Rec_ID = str_pad(Rec_ID, width = 3, pad = "0")) %>%
  unite("Recipient", Rec_Pop, Rec_ID, sep = "_") %>%
  
  # Separate and pad the 'Donor' column
  separate(Donor, into = c("Donor_Pop", "Donor_ID"), sep = "_") %>%
  mutate(Donor_ID = str_pad(Donor_ID, width = 3, pad = "0")) %>%
  unite("Donor", Donor_Pop, Donor_ID, sep = "_")

```
Load in datasets from all populations
```{r}
...
```{r}
# Merge the 'g0_msm' column from G0_msm into AC_fit_22 based on Recipient and Donor
AC_fit_22 <- AC_fit_22 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into BB_fit_22 based on Recipient and Donor
BB_fit_22 <- BB_fit_22 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into BO_fit_22 based on Recipient and Donor
BO_fit_22 <- BO_fit_22 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into HR_fit_22 based on Recipient and Donor
HR_fit_22 <- HR_fit_22 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))
...
```{r}
# Merge the 'g0_msm' column from G0_msm into AC_fit_22 based on Recipient and Donor
AC_fit_23_G1 <- AC_fit_23_G1 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into BB_fit_22 based on Recipient and Donor
BB_fit_23_G1 <- BB_fit_23_G1 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into BO_fit_22 based on Recipient and Donor
BO_fit_23_G1 <- BO_fit_23_G1 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))

# Merge the 'g0_msm' column from G0_msm into HR_fit_22 based on Recipient and Donor
HR_fit_23_G1 <- HR_fit_23_G1 %>%
  left_join(G0_msm %>% select(Recipient, Donor, g0_msm), by = c("Recipient", "Donor"))
```
...
```{r}
# Save as a CSV file
write_csv(AC_fit_22, here::here("data_sheets", "compiled_sheets", "AC_fit_22.csv"))

write_csv(BB_fit_22, here::here("data_sheets", "compiled_sheets", "BB_fit_22.csv"))

write_csv(BO_fit_22, here::here("data_sheets", "compiled_sheets", "BO_fit_22.csv"))

write_csv(HR_fit_22, here::here("data_sheets", "compiled_sheets", "HR_fit_22.csv"))


write_csv(AC_fit_23_G1, here::here("data_sheets", "compiled_sheets", "AC_fit_23_G1.csv"))

write_csv(BB_fit_23_G1, here::here("data_sheets", "compiled_sheets", "BB_fit_23_G1.csv"))

write_csv(BO_fit_23_G1, here::here("data_sheets", "compiled_sheets", "BO_fit_23_G1.csv"))

write_csv(HR_fit_23_G1, here::here("data_sheets", "compiled_sheets", "HR_fit_23_G1.csv"))


write_csv(AC_fit_23_G2, here::here("data_sheets", "compiled_sheets", "AC_fit_23_G2.csv"))

write_csv(BB_fit_23_G2, here::here("data_sheets", "compiled_sheets", "BB_fit_23_G2.csv"))

write_csv(BO_fit_23_G2, here::here("data_sheets", "compiled_sheets", "BO_fit_23_G2.csv"))

write_csv(HR_fit_23_G2, here::here("data_sheets", "compiled_sheets", "HR_fit_23_G2.csv"))
```


```{r}
# Save as a CSV file
write_csv(AC_mean_donor_traits_22, here::here("data_sheets", "compiled_sheets", "AC_mean_donor_traits_22.csv"))

write_csv(BB_mean_donor_traits_22, here::here("data_sheets", "compiled_sheets", "BB_mean_donor_traits_22.csv"))

write_csv(BO_mean_donor_traits_22, here::here("data_sheets", "compiled_sheets", "BO_mean_donor_traits_22.csv"))

write_csv(HR_mean_donor_traits_22, here::here("data_sheets", "compiled_sheets", "HR_mean_donor_traits_22.csv"))


write_csv(AC_mean_donor_traits_23_G1, here::here("data_sheets", "compiled_sheets", "AC_mean_donor_traits_23_G1.csv"))

write_csv(BB_mean_donor_traits_23_G1, here::here("data_sheets", "compiled_sheets", "BB_mean_donor_traits_23_G1.csv"))

write_csv(BO_mean_donor_traits_23_G1, here::here("data_sheets", "compiled_sheets", "BO_mean_donor_traits_23_G1.csv"))

write_csv(HR_mean_donor_traits_23_G1, here::here("data_sheets", "compiled_sheets", "HR_mean_donor_traits_23_G1.csv"))


write_csv(AC_mean_donor_traits_23_G2, here::here("data_sheets", "compiled_sheets", "AC_mean_donor_traits_23_G2.csv"))

write_csv(BB_mean_donor_traits_23_G2, here::here("data_sheets", "compiled_sheets", "BB_mean_donor_traits_23_G2.csv"))

write_csv(BO_mean_donor_traits_23_G2, here::here("data_sheets", "compiled_sheets", "BO_mean_donor_traits_23_G2.csv"))

write_csv(HR_mean_donor_traits_23_G2, here::here("data_sheets", "compiled_sheets", "HR_mean_donor_traits_23_G2.csv"))
