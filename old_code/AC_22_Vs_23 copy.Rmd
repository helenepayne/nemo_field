...
Now compare Generation 1 from AC in both 2022 and 2023, but first we need to determine which genotypes are the same in 2022 and 2023
...
datasets <- list(
  "2022" = AC_common_22_23_G1_22,
  "2023_G1" = AC_common_22_23_G1_23
)
...
for (trait in traits) {
  ...
  # Create the histogram
  ggplot(AC_common_22_23_G1, aes_string(x = trait, fill = "Year")) +
    geom_histogram(position = "identity", alpha = 0.6, binwidth = 0.2) +
  ...
  # Split the data by year
  trait_2022 <- na.omit(AC_common_22_23_G1_22[[trait]])
  trait_2023 <- na.omit(AC_common_22_23_G1_23[[trait]])
  ...
  # Perform Levene's test for homogeneity of variances
  levene_test <- leveneTest(as.formula(paste(trait, "~ as.factor(Year)")), data = AC_common_22_23_G1)
  print(paste("Levene's test for", trait, ":"))
  print(levene_test)
  
  # Perform Mann-Whitney U test (Wilcoxon rank-sum test)
  mann_whitney_test <- wilcox.test(as.formula(paste(trait, "~ Year")), data = AC_common_22_23_G1)
  print(paste("Mann-Whitney U test for", trait, ":"))
  print(mann_whitney_test)
  
  # Optionally perform t-test if data is normally distributed and variances are equal
  #if (shapiro_test_2022$p.value > 0.05 && shapiro_test_2023$p.value > 0.05 && levene_test$`Pr(>F)`[1] > 0.05) {
    t_test_result_year <- t.test(as.formula(paste(trait, "~ Year")), data = AC_common_22_23_G1)
    print(paste("t-test for", trait, ":"))
    print(t_test_result_year)
  } #else {
    #print(paste("Skipping t-test for", trait, "due to non-normal distribution or unequal variances"))
...
Any difference would be due to either the effect of greenhouse produced seeds or a generation of dplyr::selection
...
# Define the datasets and the years they correspond to
datasets <- list(
  "2023_G1" = AC_common_23_G1,
  "2023_G2" = AC_common_23_G2
)
...
for (trait in traits) {
  ...
  # Create the histogram
  ggplot(AC_common_23_G1_G2, aes_string(x = trait, fill = "Gen")) +
    geom_histogram(position = "identity", alpha = 0.6, binwidth = 0.2) +
    labs(title = paste("Comparison of", trait),
         x = trait,
         y = "Frequency") +
  ...
  # Split the data by generation and ensure numeric conversion
  trait_G1 <- na.omit(as.numeric(AC_common_23_G1[[trait]]))
  trait_G2 <- na.omit(as.numeric(AC_common_23_G2[[trait]]))
  ...
  # Perform Levene's test for homogeneity of variances
  levene_test <- leveneTest(as.formula(paste(trait, "~ as.factor(Gen)")), data = AC_common_23_G1_G2)
  print(paste("Levene's test for", trait, ":"))
  print(levene_test)
  
  # Perform Mann-Whitney U test (Wilcoxon rank-sum test)
  mann_whitney_test <- wilcox.test(as.formula(paste(trait, "~ Gen")), data = AC_common_23_G1_G2)
  print(paste("Mann-Whitney U test for", trait, ":"))
  print(mann_whitney_test)
  
  # Perform t-test
  t_test_result_gen <- t.test(as.formula(paste(trait, "~ Gen")), data = AC_common_23_G1_G2)
  print(paste("t-test for", trait, ":"))
  print(t_test_result_gen)
}
