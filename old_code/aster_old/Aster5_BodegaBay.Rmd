...
#cleaning up the data when I read it in
...
map.factory <- function(rout, is.subsamp) {
  ...
  #modmat <- rout$obj$modmat[1, , , drop = FALSE] #1st column in modmat (edited with Anna 5/16 bc this should not be my root)- removed 5/16
  # # set root = 8
  #root <- array(8, dim = dim(modmat)[1:2]) #set root array to 8 (edited with Anna 5/16)- removed 5/16
  ...
  function (b) {
    ...
    xi <- predict(
      aout,
      #x = x, # added- removed 5/16
      newcoef = alpha,
      #modmat = modmat,- removed 5/16
      #root = root,- removed 5/16
...
map.factory <- function(rout, is.subsamp) {
  ...
  if (length(ifit) != 1)
    stop("no fixed effect named fit")
  # modmat <- rout$obj$modmat[5, , , drop = FALSE] #commented this out with Anna
  # # set root = 5
  # root <- array(1, dim = dim(modmat)[1:2]) #commented this out with Anna
  ...
  function (b) {
    ...
    xi <- predict(
      aout,
      # x = x, # added #commented this out with Anna
      newcoef = alpha,
      # modmat = modmat, #commented this out with Anna
      # root = root, #commented this out with Anna
      model.type = "conditional",
      is.always.parameter = TRUE)
