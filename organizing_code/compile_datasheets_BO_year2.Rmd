...
skel_weights_BO_2 <- skel_weights_BO %>% 
  rename(skel_dryweights_mg_SEG = 7) %>% 
  select(-Plant_ID)

#Check that IDs line up
skelcheck <- merge(BO_Fitness_2, skel_weights_BO_2, by = c("Transect", "SegPos", "Gen", "Recipient"), all.x = TRUE)
...
BO_merged_1 <- merge(BO_flowering, skel_weights_BO, by = c("Transect", "SegPos", "Gen", "Recipient", "Plant_ID"), all.x = TRUE) %>% select(-Fitness) %>% arrange(Transect, Sequence, SegPos)

#merging the skeleton data for the entire segment
BO_merged_1.5 <- merge(BO_merged_1, skel_weights_BO_2, by = c("Transect", "SegPos", "Gen", "Recipient"), all.x = TRUE) %>% select(-Fitness)%>% arrange(Transect, Sequence, SegPos)

####<-left off here
BO_merged_2 <- merge(BO_merged_1.5, BO_fruit_seeds_2, by = c("Sequence", "Transect", "SegPos", "Gen", "Recipient", "Donor", "Plant_ID"), all.x = TRUE) %>% select(-c(FD_01:FD_05, N_CLOSED_fruit_D1:N_total_seeds_D6, Notes))  %>% arrange(Transect, Sequence, SegPos) %>% 
  select(Sequence, Transect, SegPos, Gen, Donor, Recipient, Plant_ID, F_plant, everything(), -32)
