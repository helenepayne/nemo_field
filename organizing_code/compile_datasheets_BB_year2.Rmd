...
BB_merged_1.5 <- merge(BB_merged_1, skel_weights_BB_3, by = c("Location", "SegPos", "Gen", "Recipient"), all.x = TRUE) %>% select(-Fitness)%>% arrange(Location, Sequence, SegPos)

BB_merged_2 <- merge(BB_merged_1.5, BB_fruit_seeds_2, by = c("Sequence", "Location", "SegPos", "Gen", "Recipient", "Donor", "Plant_ID"), all.x = TRUE) %>% select(-c(FD_01:FD_05, N_CLOSED_fruit_D1:N_total_seeds_D8))  %>% arrange(Location, Sequence, SegPos) %>% 
  select(Sequence, Location, SegPos, Gen, Donor, Recipient, Plant_ID, F_plant, everything(), -37)
