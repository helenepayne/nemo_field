...
HR_merged_1 <- merge(HR_flowering, skel_weights_HR, by = c("Transect", "SegPos", "Gen", "Recipient", "Plant_ID"), all.x = TRUE) %>% select(-c(Fitness, FD_01:FD_05, LeafD, FLR_P, Skeleton, Notes)) %>% arrange(Transect, Sequence, SegPos)

#merging the skeleton data for the entire segment
HR_merged_1.5 <- merge(HR_merged_1, skel_weights_HR_2, by = c("Transect", "SegPos", "Gen", "Recipient"), all.x = TRUE) %>% select(-Fitness)%>% arrange(Transect, Sequence, SegPos)
