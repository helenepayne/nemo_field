...
f_AClong_3 <- f_AClong_2 %>% 
  ...
  mutate(Fl_4.9 = case_when(Fl_4.9 == "2022-04-09" ~ "2022-04-09",#assuming 4/21 and 4/22 are the same flowering date
...
AC_seedct_test <- read_csv(here::here("data_sheets", "fruit_seed_counts_final", "AC-seed-count_2023-02-05.csv"))
...
#Merge
AC_merged_df_fl3 <- merge(AC_merged_df_fl2, Nemo_d13_AC2, by = c("Recipient", "Transect", "Plant_ID"), all.x = TRUE) #add Plant_ID to merge by
...
test1 <- skel_weights_seg1 %>% count(FitP, Rep_FitP)
test2 <- skel_weights_seg1 %>% count(FitP, Rep_FitP, any_FitP)
...
skel_weights_unduped1 <- skel_weights_duped %>% 
  filter(case_when(duped == TRUE & Replicated == "R" & Block != "W" ~ FitP == TRUE,
          # If a row is duplicated, represents a segment with replicates, AND is in an UNwatered block, then the statement FitP == TRUE is evaluated as either TRUE or FALSE, which would be the output value.  If FitP = TRUE, filtering will RETAIN these rows.  These will represent the originally assiged Fitness plant when replicates (Non-fitness plants) are available for the segment
...
write_csv(AC_mastersheet_v3, here::here("data_sheets", "compiled_sheets", "AC_mastersheet_full_2024-01-28.csv"))
...
write_csv(AC_mastersheet_F_mains, here::here("data_sheets", "compiled_sheets", "AC_mastersheet_Fitness-mains_2024-01-28.csv"))

write_csv(AC_mastersheet_F_all, here::here("data_sheets", "compiled_sheets", "AC_mastersheet_Fitness-all_2024-01-28.csv"))
```
