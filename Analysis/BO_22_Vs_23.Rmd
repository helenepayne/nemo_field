...
BO_22_23_fit$Genotype <- paste(BO_22_23_fit$Donor, BO_22_23_fit$Recipient, sep = "-")
...
BO_22_23_fit <- BO_22_23_fit %>%
  left_join(BO_prop_sample %>% select(Year, Gen, Transect, Sequence, Donor, Recipient, SegPos, prop_surv_to_flower),
...
BO_22_fit <- BO_22_23_fit %>%
  filter(Year == 2022)

BO_23_fit_G1 <- BO_22_23_fit %>%
  filter(Year == 2023) %>%
  filter(Gen == "G1")

BO_23_fit_G2 <- BO_22_23_fit %>%
  filter(Year == 2023) %>%
  filter(Gen == "G2")
...
Now compare Generation 1 from BO in both 2022 and 2023, but first we need to determine which genotypes are the same in 2022 and 2023

```{r}
# Create a dataframe with common "Genotype" values for 2022
BO_common_22_23_G1_22 <- semi_join(BO_22_fit, BO_23_fit_G1, by = "Genotype")

# Create a dataframe with common "Genotype" values for 2022
BO_common_22_23_G1_23 <- semi_join(BO_23_fit_G1, BO_22_fit, by = "Genotype")

# Merge these two dataframes to have one finalized dataset to compare G1 2022 and G1 2023
BO_common_22_23_G1 <- full_join(BO_common_22_23_G1_22, BO_common_22_23_G1_23, by = intersect(names(BO_common_22_23_G1_22), names(BO_common_22_23_G1_23)))
```
...
for (trait in traits) {
  ...
  # Calculate the mean trait value for each maternal (Recipient) family for 2022
  maternal_means_22 <- BO_common_22_23_G1_22 %>%
    group_by(Recipient, Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE))
  
  
  # Calculate the mean trait value for each maternal (Recipient) family for 2023 G1
  maternal_means_23_G1 <- BO_common_22_23_G1_23 %>%
    group_by(Recipient, Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE))
...
for (trait in traits) {
  ...
  # Calculate the mean trait value for each paternal (Donor) family for 2022
  paternal_means_22 <- BO_common_22_23_G1_22 %>%
    group_by(Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE))
  
  # Calculate the mean trait value for each paternal (Donor) family for 2023 G1
  paternal_means_23_G1 <- BO_common_22_23_G1_23 %>%
    group_by(Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE))
...
datasets <- list(
  "2022" = BO_common_22_23_G1_22,
  "2023_G1" = BO_common_22_23_G1_23
)
...
for (trait in traits) {
  ...
  if (!is.factor(BO_common_22_23_G1$Year)) {
    BO_common_22_23_G1$Year <- as.factor(BO_common_22_23_G1$Year)
  }
  
  # Create the histogram
  ggplot(BO_common_22_23_G1, aes_string(x = trait, fill = "Year")) +
    geom_histogram(position = "identity", alpha = 0.6, binwidth = 0.2) +
  ...
  # Split the data by year
  trait_2022 <- na.omit(BO_common_22_23_G1_22[[trait]])
  trait_2023 <- na.omit(BO_common_22_23_G1_23[[trait]])
  ...
  # Perform Levene's test for homogeneity of variances
  levene_test <- leveneTest(as.formula(paste(trait, "~ as.factor(Year)")), data = BO_common_22_23_G1)
  print(paste("Levene's test for", trait, ":"))
  print(levene_test)
  
  # Perform Mann-Whitney U test (Wilcoxon rank-sum test)
  mann_whitney_test <- wilcox.test(as.formula(paste(trait, "~ Year")), data = BO_common_22_23_G1)
  print(paste("Mann-Whitney U test for", trait, ":"))
  print(mann_whitney_test)
  
  # Optionally perform t-test if data is normally distributed and variances are equal
  #if (shapiro_test_2022$p.value > 0.05 && shapiro_test_2023$p.value > 0.05 && levene_test$`Pr(>F)`[1] > 0.05) {
    t_test_result_year <- t.test(as.formula(paste(trait, "~ Year")), data = BO_common_22_23_G1)
    print(paste("t-test for", trait, ":"))
    print(t_test_result_year)
  } #else {
    #print(paste("Skipping t-test for", trait, "due to non-normal distribution or unequal variances"))
...
for (trait in traits) {
  ...
  BO_common_23_G1 <- semi_join(BO_23_fit_G1, BO_23_fit_G2, by = "Genotype")
  BO_common_23_G2 <- semi_join(BO_23_fit_G2, BO_23_fit_G1, by = "Genotype")
  
  # Merge these two dataframes to have one finalized dataset to compare G1 2023 and G2 2023
  BO_common_23_G1_G2 <- full_join(BO_common_23_G1, BO_common_23_G2, by = intersect(names(BO_common_23_G1), names(BO_common_23_G2)))

  # Calculate the mean trait value for each maternal (Recipient) family for G1, 2023
  maternal_means_23_G1 <- BO_common_23_G1 %>%
    group_by(Recipient, Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE)) %>%
    mutate(Gen = "1")
  
  # Calculate the mean trait value for each maternal (Recipient) family for G2, 2023
  maternal_means_23_G2 <- BO_common_23_G2 %>%
    group_by(Recipient, Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE)) %>%
    mutate(Gen = "2")
...
for (trait in traits) {
  ...
  # Calculate the mean trait value for each paternal (Donor) family for G1, 2023
  paternal_means_23_G1 <- BO_common_23_G1 %>%
    group_by(Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE), .groups = "drop") %>%
    mutate(Gen = "1")
  
  # Calculate the mean trait value for each paternal (Donor) family for G2, 2023
  paternal_means_23_G2 <- BO_common_23_G2 %>%
    group_by(Donor) %>%
    summarise(mean_trait = mean(get(trait), na.rm = TRUE), .groups = "drop") %>%
    mutate(Gen = "2")
...
# Define the datasets and the years they correspond to
datasets <- list(
  "2023_G1" = BO_common_23_G1,
  "2023_G2" = BO_common_23_G2
)
...
for (trait in traits) {
  ...
  if (!is.factor(BO_common_23_G1_G2$Gen)) {
    BO_common_23_G1_G2$Gen <- as.factor(BO_common_23_G1_G2$Gen)
  }
  
  # Create the histogram
  ggplot(BO_common_23_G1_G2, aes_string(x = trait, fill = "Gen")) +
    geom_histogram(position = "identity", alpha = 0.6, binwidth = 0.2) +
    labs(title = paste("Comparison of", trait),
         x = trait,
         y = "Frequency") +
  ...
  # Split the data by generation and ensure numeric conversion
  trait_G1 <- na.omit(as.numeric(BO_common_23_G1[[trait]]))
  trait_G2 <- na.omit(as.numeric(BO_common_23_G2[[trait]]))
  ...
  # Perform Levene's test for homogeneity of variances
  levene_test <- leveneTest(as.formula(paste(trait, "~ as.factor(Gen)")), data = BO_common_23_G1_G2)
  print(paste("Levene's test for", trait, ":"))
  print(levene_test)
  
  # Perform Mann-Whitney U test (Wilcoxon rank-sum test)
  mann_whitney_test <- wilcox.test(as.formula(paste(trait, "~ Gen")), data = BO_common_23_G1_G2)
  print(paste("Mann-Whitney U test for", trait, ":"))
  print(mann_whitney_test)
  
  # Perform t-test
  t_test_result_gen <- t.test(as.formula(paste(trait, "~ Gen")), data = BO_common_23_G1_G2)
  print(paste("t-test for", trait, ":"))
  print(t_test_result_gen)
}
